...
MODELS = {
    "چت متنی": {
        # --- The complete, un-merged list of text models ---
        ...
        "Gemini 2.5 Flash-Lite (بهینه | ۱۵ درخواست در دقیقه | ۱۰۰۰ در روز)": "gemini-1.5-flash-latest", # UI distinction, same backend model
        "Gemini 2.0 Flash (پایدار | ۱۵ درخواست در دقیقه | ۲۰۰ در روز)": "gemini-1.0-pro",
        "Gemini 2.0 Flash-Lite (سهمیه بالا | ۳۰ درخواست در دقیقه | ۲۰۰ در روز)": "gemini-1.0-pro-002", # Using a slightly different ID for distinction
    },
    "تولید تصویر": {
...
async def stream_gemini_response(api_msgs: List[Dict], model: str) -> Generator[str, Any, None]:
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        try:
            ...
            yield f"**خطا:** سهمیه رایگان تمام شده یا خطایی در API رخ داده است. لطفاً مدل دیگری را امتحان کنید.\n\n`{e.response.text}`"
...
sidebar_ui() # Calling the new refactored sidebar function
...
# Respond if the last message was from the user
